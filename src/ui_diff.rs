...
fn render_diff_status_bar(
    ...
    let status = if let Some(msg) = status_message {
        ...
        match editor_state {
            EditorState::Idle => {
                ...
                format!(
                    " DIFF | {} | Line {}/{}{}  ^G Help  ^X Exit  ^O Save  ^W Search  ^K Exit Diff  ^T Theme  ^D Del  ^N/^P Jump",
                    file,
                    cursor_line + 1,
                    total_lines,
                    modified_flag
            ...
            EditorState::Searching { query, .. } => {
                format!(" Search: {}█  Enter: Next  Esc: Cancel", query)
            }
    ...
    queue!(
        ...
        Print(format!(
            "{:width$}",
            status.chars().take(width as usize - 1).collect::<String>(),
